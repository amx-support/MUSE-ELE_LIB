...
class TcpServer:
    ...
    # 初期化 -----------------------------------------------------------------------------------------
    def __init__(self, device:str, port:int, receive_bytes = 1024, keepidle = 30, keepintval = 5, keepcnt = 3, sendtimeout = 10000):
        ...
        self.__s_socket = None                          # サーバーソケット (待ち受用)
        self.__c_socket = None                          # クライアントソケット (相手との通信用)
        self.__c_address = None                         # クライアント側IPアドレス
        self.__c_port = None                            # クライアント側IPポート
        self.__s_port = port                            # サーバー側IPポート
    ...
    def open(self):
        ...
        if self.__online == False:
            ...
            if self.__s_socket is None:
                ...
                # 受信ポートをバインド
                try:
                    self.__s_socket.bind(("0.0.0.0", self.__s_port))
    ...
    # 待ち受け処理（スレッド用） -------------------------------------------------------------------------------
    def __connect_listen(self):
        # ソケットが開いていたら
        if self.__s_socket is not None:
            # タイムアウトを１秒に設定
            self.__s_socket.settimeout(1)

            # 待ち受け開始
            self.__s_socket.listen(1)

            # 接続
            while True:
                try:
                    # クライアント側が接続
                    sock, addr = self.__s_socket.accept()
                    self.__c_socket = sock
                    self.__c_address = addr[0]
                    self.__c_port = addr[1]
                    self.__c_socket.setsockopt(socket.SOL_SOCKET, socket.SO_KEEPALIVE, 1)
                    self.__c_socket.setsockopt(socket.IPPROTO_TCP, socket.TCP_KEEPIDLE, self.__keepidle)
                    self.__c_socket.setsockopt(socket.IPPROTO_TCP, socket.TCP_KEEPINTVL, self.__keepintval)
                    self.__c_socket.setsockopt(socket.IPPROTO_TCP, socket.TCP_KEEPCNT, self.__keepcnt)
                    self.__c_socket.setsockopt(socket.IPPROTO_TCP, socket.TCP_USER_TIMEOUT, self.__sendtimeout)
    ...
    def __connect(self):
        ...
        if self.__s_socket is not None:
            # 待ち受けスレッド開始
            self.__connect_thread = threading.Thread(target=self.__connect_listen)
    ...
    def __close(self):
        ...
        if self.__s_socket is not None:
            ...
            # 待ち受け用スレッドを終了
            if self.__connect_thread is not None:
                self.__connect_thread = None

        # クライアント用ソケットが開いていれば
        if self.__c_socket is not None:
            self.__c_socket.close()
            self.__c_socket = None
            self.__c_address = None
            self.__c_port = None
            self.__online = False
    ...
    def __recv(self):
        ...
        if self.__c_socket is not None:
            # タイムアウトを１秒に
            self.__c_socket.settimeout(1)

            # 受信ループ
            while True:
                try:
                    # 受信待ち受け
                    data, address = self.__c_socket.recvfrom(self.__receive_bytes)
                    ...
                    if not data:
                        ...
                        # 受信イベント生成
                        self.__ev_receive(data, (self.__c_address, self.__c_port))
    ...
    def __ev_online(self):
        if self.__online_callback_func is not None:
            e = EvParams(self, self.__device, "online", f"{self.__class__.__name__}/online", True, {"address":self.__c_address, "port":self.__c_port})
