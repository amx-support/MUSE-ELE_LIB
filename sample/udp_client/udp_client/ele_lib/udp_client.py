...
class UdpClient:
    ...
    # 初期化 -----------------------------------------------------------------------------------------
    def __init__(self, device:str, address:str, send_port:int, recv_port=None, receive_bytes = 1024):
        ...
        # 送信ポート番号がintでなければ例外をスロー
        if type(send_port) is not int:
            raise ValueError(f"send_port<{send_port}({type(send_port).__name__})> is not int")
        ...
        self.__send_port = send_port                # 送信用IPポート番号
    ...
    def open(self):
        ...
        # オフラインなら
            ...
            if self.__socket is None:
                ...
                # 受信ありなら
                if self.__recv_port is not None:
                    try:
                        # 受信ポートをバインド
    ...
    def __send(self):
        ...
        if self.__socket is not None:
            ...
            while self.__command_queue.qsize() > 0:
                ...
                self.__socket.sendto(cmd,(self.__address, self.__send_port))
